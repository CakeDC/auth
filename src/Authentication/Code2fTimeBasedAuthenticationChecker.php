 ...
 */
class Code2fTimeBasedAuthenticationChecker implements Code2fAuthenticationCheckerInterface
{
    /**
    ...
    public function isRequired(?array $user = null, ServerRequestInterface $request)
    {
        ...
        return !empty($user) &&
            (
                empty($latestOtpCode) ||
                ((new FrozenTime())->diffInDays($latestOtpCode->validated) >= Configure::read('Code2f.daysBeforeVerifyAgain', 15))
            ) &&
            $this->isEnabled();
    }
}
